...
function buildOrderText(form: FormState, lines: ReturnType<typeof useCart>['lines'], subtotal: number): string {
  ...
  return [
    ...
    'ITEMS',
    itemLines,
    `Subtotal (estimated): ${formatPrice(subtotal)}`,
    '',
    'DELIVER TO',
    form.name,
    form.address1,
...
export function Checkout() {
  ...
  if (confirmedOrderText) {
    return (
      <section className="mx-auto max-w-2xl px-6 py-40 text-center">
        ...
        <h1 className="mt-8 font-display text-4xl text-white sm:text-5xl">Order details ready</h1>
        <p className="mx-auto mt-4 max-w-md font-body text-floe-ice/70">
          Your email app should have opened with your order pre-filled to <strong className="text-white">{contact.email}</strong> — hit
          send there to confirm. If it didn't open, copy the details below and send them yourself.
        </p>

        <div className="mt-8 rounded-2xl border border-white/10 bg-ink-700 p-6 text-left">
          <pre className="whitespace-pre-wrap font-body text-sm text-floe-ice/80">{confirmedOrderText}</pre>
        </div>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-4">
          <button
            type="button"
            onClick={copyOrder}
            className="flex items-center gap-2 rounded-full border border-white/20 px-6 py-3 font-body text-sm uppercase tracking-[0.15em] text-white hover:border-floe-glacier"
          >
        ...
        <p className="mt-10 text-xs text-floe-ice/40">
  ...
  if (cart.lines.length === 0) {
    return (
      <section className="mx-auto max-w-lg px-6 py-40 text-center">
        ...
        <p className="mt-4 font-body text-floe-ice/70">Add something from the range before checking out.</p>
        <Link
          to="/"
  ...
  const inputClass = (field: keyof FormState) =>
    `w-full rounded-xl border bg-white/[0.03] px-4 py-3 font-body text-white placeholder:text-floe-ice/30 focus:outline-none focus:ring-1 ${
      errors[field] ? 'border-floe-red focus:ring-floe-red' : 'border-white/15 focus:ring-floe-glacier'
    }`

  return (
    <section className="mx-auto max-w-6xl px-6 py-32 sm:px-10">
      <Link to="/" className="inline-flex items-center gap-2 font-body text-sm text-floe-ice/60 hover:text-white">
        <ArrowLeft className="h-4 w-4" />
        Continue shopping
      </Link>

      <h1 className="mt-6 font-display text-5xl text-white">Checkout</h1>

      <div className="mt-12 grid gap-12 lg:grid-cols-[1.4fr_1fr]">
        <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
          <div className="grid gap-5 sm:grid-cols-2">
            <div>
              <label className="mb-1.5 block font-body text-xs uppercase tracking-widest text-floe-mist">Full name</label>
            ...
            <div>
              <label className="mb-1.5 block font-body text-xs uppercase tracking-widest text-floe-mist">Email</label>
          ...
          <div>
            <label className="mb-1.5 block font-body text-xs uppercase tracking-widest text-floe-mist">Phone</label>
          ...
          <div>
            <label className="mb-1.5 block font-body text-xs uppercase tracking-widest text-floe-mist">Address</label>
          ...
          <div className="grid gap-5 sm:grid-cols-3">
            <div>
              <label className="mb-1.5 block font-body text-xs uppercase tracking-widest text-floe-mist">City</label>
            ...
            <div>
              <label className="mb-1.5 block font-body text-xs uppercase tracking-widest text-floe-mist">Postal code</label>
            ...
            <div>
              <label className="mb-1.5 block font-body text-xs uppercase tracking-widest text-floe-mist">Country</label>
          ...
          <div>
            <label className="mb-1.5 block font-body text-xs uppercase tracking-widest text-floe-mist">Delivery notes (optional)</label>
          ...
          <p className="text-center text-xs text-floe-ice/40">
          ...
          <ul className="mt-5 flex flex-col gap-4">
            {cart.lines.map((line) => (
              <li key={line.productId} className="flex justify-between gap-3 text-sm">
                <span className="text-floe-ice/80">
                  {line.name} <span className="text-floe-ice/40">× {line.qty}</span>
                </span>
                <span className="whitespace-nowrap text-white">{formatPrice(line.price * line.qty)}</span>
              </li>
            ))}
          </ul>
          <div className="mt-5 flex justify-between border-t border-white/10 pt-4 font-body text-white">
            <span>Subtotal</span>
            <span className="font-display text-lg">{formatPrice(cart.subtotal)}</span>
          </div>
          <p className="mt-1 text-[10px] uppercase tracking-wide text-floe-ice/40">Estimated — pending confirmed pricing</p>
        </aside>
      </div>
    </section>
  )
